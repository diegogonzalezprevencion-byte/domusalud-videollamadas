  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/login', {
        ...
        cache: 'no-store',
        body: JSON.stringify({ username: username.trim().toLowerCase(), role, password }),
      });
      const result: { accessToken?: string; refreshToken?: string; error?: string } = await response.json();
      if (!response.ok || !result.accessToken || !result.refreshToken) {
        throw new Error(result.error || 'No se pudo ingresar. Revisa tus datos.');
      }
    ...
    <main className="auth-page">
      <div className="auth-art">
        ...
        <div className="auth-art-main"><span className="eyebrow light">TU ESPACIO DE TRABAJO</span>
          ...
          <p>Dos espacios de trabajo: administración global y agenda personal de cada socio.</p>
          <div className="art-chips"><span>▦ Calendario</span><span>◉ Videollamadas</span><span>▤ Minutas</span></div>
        </div>
        <div className="auth-art-footer">Domus Salud · Acceso con perfiles separados</div>
      </div>
      <div className="auth-right">
        <form className="auth-form" onSubmit={submit}>
          <span className="eyebrow">BIENVENIDO A DOMUS</span>
          <h2>Inicia sesión</h2>
          <p className="muted">Selecciona tu perfil antes de ingresar.</p>
          <div className="role-selector" role="group" aria-label="Tipo de perfil">
            <button type="button" aria-pressed={role === 'usuario'} className={role === 'usuario' ? 'role-choice chosen-role' : 'role-choice'} onClick={() => { setRole('usuario'); setError(''); }}>♧ Usuario</button>
            <button type="button" aria-pressed={role === 'admin'} className={role === 'admin' ? 'role-choice chosen-role' : 'role-choice'} onClick={() => { setRole('admin'); setError(''); }}>⚙ Administrador</button>
          </div>
          <label>Nombre de usuario<input required minLength={2} maxLength={80} value={username} onChange={e => setUsername(e.target.value)} placeholder="dgonzalez" autoComplete="username" /></label>
          ...
          <button disabled={working} type="submit" className="button button-primary full">{working ? 'Comprobando…' : `Ingresar como ${role === 'admin' ? 'administrador' : 'usuario'}`} →</button>
          <p className="auth-disclaimer">Las cuentas se crean de forma privada en Supabase. No hay registro público.</p>
        </form>
      </div>
    </main>
  );
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'sidebar-open' : ''}`}>
        ...
        <div className="sidebar-bottom"><span className="connection-dot" /> {isAdmin ? 'Perfil administrador' : 'Perfil usuario'} <small>Domus Salud · V2</small></div>
      </aside>
      <div className="main-column">
        ...
        <main className="content">
          ...
          {tab === 'inicio' && <>
            ...
            <div className="stat-grid"><div className="stat-card"><span className="stat-icon mint">▦</span><span className="stat-label">{isAdmin ? 'Próximas reuniones globales' : 'Mis próximas reuniones'}</span><strong>{upcoming.length}</strong><small>En tu agenda</small></div>
          ...
          {tab === 'reuniones' && <><PageHeading overline="ENCUENTROS" title="Mis reuniones" desc={isAdmin ? "Consulta todas las reuniones y administra sus minutas." : "Consulta tus reuniones, participantes y enlaces privados."} onCreate={() => newMeeting()} />
          ...
          {tab === 'socios' && <><PageHeading overline="PERSONAS" title="Socios y participantes" desc="Usuarios registrados que puedes invitar a reuniones." />
            <section className="panel directory"><div className="directory-top"><strong>Directorio del equipo</strong><span className="badge">{profiles.length} personas</span></div>
              ...
              <p className="directory-help">Las cuentas se crean desde Supabase → Authentication → Users. Solo un administrador configura los roles mediante el SQL de instalación. Las invitaciones por correo están pendientes.</p>
            </section></>}
