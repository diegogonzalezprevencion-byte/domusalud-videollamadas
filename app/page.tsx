...
type Tab = 'inicio' | 'calendario' | 'reuniones' | 'socios' | 'minutas';
...
function AuthPanel() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (register) {
        const { data, error: signError } = await sb.auth.signUp({
          email: email.trim().toLowerCase(), password,
          options: { data: { full_name: name.trim() } },
        });
        if (signError) throw signError;
        if (!data.session) setMessage('Cuenta solicitada. Revisa tu correo y confirma el registro antes de entrar.');
        else setMessage('Cuenta creada. Abriendo tu espacio…');
      } else {
        const { error: signError } = await sb.auth.signInWithPassword({ email: email.trim().toLowerCase(), password });
        if (signError) throw signError;
      }
    } catch (err) {
    ...
    <main className="auth-page">
      <div className="auth-art">
        ...
        <div className="auth-art-main"><span className="eyebrow light">TU ESPACIO DE TRABAJO</span>
          ...
          <p>Un lugar privado para coordinar reuniones, hablar por video y registrar los acuerdos de Domus Salud.</p>
          <div className="art-chips"><span>▦ Calendario</span><span>◉ Videollamadas</span><span>▤ Minutas</span></div>
        </div>
        <div className="auth-art-footer">Plataforma de gestión de reuniones · Versión inicial</div>
      </div>
      <div className="auth-right">
        <form className="auth-form" onSubmit={submit}>
          <span className="eyebrow">BIENVENIDO A DOMUS</span>
          <h2>{register ? 'Crear una cuenta' : 'Inicia sesión'}</h2>
          <p className="muted">{register ? 'Registra tu cuenta para participar en reuniones internas.' : 'Ingresa con tu correo y contraseña.'}</p>
          {register && <label>Nombre y apellido<input required minLength={2} maxLength={100} value={name} onChange={e => setName(e.target.value)} placeholder="Nombre completo" autoComplete="name" /></label>}
          <label>Correo electrónico<input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="nombre@empresa.cl" autoComplete="email" /></label>
          <label>Contraseña<input required minLength={6} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Mínimo 6 caracteres" autoComplete={register ? 'new-password' : 'current-password'} /></label>
          {message && <div className="alert success">{message}</div>}
          {error && <div className="alert danger">{error}</div>}
          <button disabled={working} type="submit" className="button button-primary full">{working ? 'Un momento…' : register ? 'Registrarme' : 'Ingresar a mi espacio'} →</button>
          <button type="button" className="auth-toggle" onClick={() => { setRegister(v => !v); setError(''); setMessage(''); }}>
            {register ? '¿Ya tienes una cuenta? Iniciar sesión' : '¿Primera vez? Crear una cuenta'}
          </button>
          <p className="auth-disclaimer">Acceso destinado a personas autorizadas de Domus Salud. Para cerrar nuevos registros, desactiva Signups en Supabase después de crear las cuentas del equipo.</p>
        </form>
      </div>
    </main>
  );
...
function MeetingDetail({ meeting, profiles, participants, selfId, onClose, onChanged }: {
  meeting: Meeting; profiles: Profile[]; participants: Participant[]; selfId: string;
  onClose: () => void; onChanged: () => Promise<void>;
}) {
  const [notes, setNotes] = useState(meeting.notes || '');
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState('');
  const isOwner = meeting.created_by === selfId;
    ...
    <section className="modal detail-modal" role="dialog" aria-modal="true" aria-labelledby="meeting-detail-title">
      ...
      <div className="detail-body">
        ...
        <div className="notes-section"><h3>Minuta de la reunión</h3><p className="helper">En esta versión, el organizador escribe los acuerdos manualmente. La IA y la grabación se agregarán después.</p>
          ...
          {!isOwner && <p className="helper">Solo quien organizó la reunión puede editar la minuta.</p>}
          {message && <div className="alert success">{message}</div>}
        </div>
      </div>
    </section>
...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    ...
    const [p, m, a] = await Promise.all([
      sb.from('profiles').select('id,email,full_name,created_at').order('full_name'),
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'sidebar-open' : ''}`}>
        ...
        <nav aria-label="Menú principal" className="sidebar-menu">{menu.map(item => <button key={item.id} className={`nav-item ${tab === item.id ? 'active' : ''}`}
          onClick={() => { setTab(item.id); setMobileNav(false); }}><span className="nav-icon">{item.icon}</span>{item.label}</button>)}</nav>
        <div className="sidebar-bottom"><span className="connection-dot" /> Plataforma privada <small>Domus Salud · V1</small></div>
      </aside>
      <div className="main-column">
        <header className="topbar"><div className="row"><button className="icon-button mobile-menu" aria-label="Abrir menú" onClick={() => setMobileNav(!mobileNav)}>☰</button><span className="topbar-location">Domus Salud <span>/</span> {menu.find(item => item.id === tab)?.label}</span></div>
          <div className="topbar-actions"><span className="topbar-email">{user.email}</span><span className="avatar">{displayName.charAt(0).toUpperCase()}</span><button className="logout-button" onClick={() => void getSupabase().auth.signOut()}>Salir</button></div></header>
        <main className="content">
          ...
          {tab === 'inicio' && <>
            <div className="hero"><div><span className="eyebrow light">PANEL DE CONTROL</span><h1>Hola, {displayName.split(' ')[0]} <span>✳</span></h1><p>Organiza tu agenda, conecta a tus socios y mantén cada acuerdo a la vista.</p><button onClick={() => newMeeting()} className="button button-white">+ Crear nueva reunión</button></div><div className="hero-deco"><div className="deco-orbit">✚</div></div></div>
            <div className="stat-grid"><div className="stat-card"><span className="stat-icon mint">▦</span><span className="stat-label">Próximas reuniones</span><strong>{upcoming.length}</strong><small>En tu agenda</small></div>
              ...
              <div className="stat-card"><span className="stat-icon sky">♧</span><span className="stat-label">Socios registrados</span><strong>{profiles.length}</strong><small>Directorio privado</small></div>
            ...
            <section className="panel dashboard-meetings"><div className="section-head"><div><span className="eyebrow">TU AGENDA</span><h2>Próximas reuniones</h2></div><button className="text-button" onClick={() => setTab('reuniones')}>Ver todas →</button></div>
          ...
          {tab === 'reuniones' && <><PageHeading overline="ENCUENTROS" title="Mis reuniones" desc="Consulta tus reuniones, participantes y enlaces privados." onCreate={() => newMeeting()} />
          ...
          {tab === 'socios' && <><PageHeading overline="PERSONAS" title="Socios y participantes" desc="Usuarios registrados que puedes invitar a reuniones." />
            <section className="panel directory"><div className="directory-top"><strong>Directorio del equipo</strong><span className="badge">{profiles.length} personas</span></div>
              {profiles.map(p => <div className="directory-person" key={p.id}><span className="avatar person-avatar">{(p.full_name || p.email).charAt(0).toUpperCase()}</span><div><strong>{p.full_name || p.email}</strong><small>{p.email}</small></div>{p.id === user.id && <span className="badge">Tú</span>}</div>)}
              <p className="directory-help">Para agregar otro socio, esa persona debe registrarse con su correo desde la pantalla de acceso. Las invitaciones automáticas por correo se incorporarán en una etapa posterior.</p>
            </section></>}

          {tab === 'minutas' && <><PageHeading overline="DOCUMENTACIÓN" title="Minutas" desc="Acuerdos guardados por los organizadores de reuniones." />
            <section className="panel list-panel">{noted.length ? noted.map(m => <button className="minutes-item" key={m.id} onClick={() => setSelected(m)}><span className="stat-icon sky">▤</span><span><strong>{m.title}</strong><small>{formatDate(m.starts_at)}</small><p>{m.notes?.slice(0, 180)}{(m.notes?.length || 0) > 180 ? '…' : ''}</p></span><b>↗</b></button>) : <div className="empty-list"><span>▤</span><strong>Sin minutas todavía</strong><p>El organizador puede redactar una minuta desde el detalle de cada reunión.</p></div>}</section></>}
        </main>
      </div>
      {formDate && <MeetingForm profiles={profiles} selfId={user.id} initialDate={formDate} onClose={() => setFormDate(null)} onCreated={refresh} />}
      {selectMeeting && <MeetingDetail key={`${selectMeeting.id}:${selectMeeting.notes || ''}`} meeting={selectMeeting} profiles={profiles} participants={participants} selfId={user.id} onClose={() => setSelected(null)} onChanged={refresh} />}
    </div>
  );
}
